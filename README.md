...
- Generated avatars expose **✦ Regenerate** on each finalized layer, with matching left/right, front/back, and connected layers included by default. The chroma-keyed Gemini boundary becomes a new layer mask by default, allowing genuinely different hair and clothing silhouettes while retaining a small attachment seam; users can opt out to keep the finalized mask exactly. Click a layer thumbnail to open a top-layer visual dropdown containing the finalized original and every accepted generated variant. Choosing one switches matching front/back, left/right, and connected group members by default; the checked group option can be disabled before selection. As soon as Gemini finishes, color-only results swap their GPU and persisted Live2D textures immediately without rebuilding the `.moc3`; the same fast path is used when selecting an older variant. Rig builds also union the alpha coverage of accepted variants into reusable ArtMeshes, so known silhouette variants such as relaxed versus flared arms switch instantly too. A genuinely new silhouette outside that cached coverage rebuilds once to expand it, then becomes fast on later switches. Right-aligned undo and redo buttons beside every layer name walk accepted generation history, remain visibly disabled when unavailable, and show per-row and status spinners during rebuilds. See-through names such as **Topwear**, **Cape**, and **Footwear Left/Right** are retained through later rebuilds. Layer controls use a compact horizontal strip that can be scrolled or dragged without sacrificing normal clicks, while **⊹ Placement** provides live X/Y pixel sliders, centering, and baseline restoration. Recovery rejects accidental full-character output and can mirror a valid paired limb when its mate fails isolation.
